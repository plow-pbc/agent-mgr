...
| Propose vendoring an upstream artifact to avoid a fetch. | Flag any ref that is **not exact**: a `sha256:` digest for the container image, a 40-char commit SHA for a git artifact (the plugin, a skill). A tag or branch silently re-points a running agent on the next upstream push, and these carry the chat token and drive a filesystem. |
