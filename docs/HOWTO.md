...
| `... builds its image but sets pull_policy: ...` | only `missing`, `never`, `build` and `if_not_present` leave a built image alone; anything else (`always`, `refresh`, `daily`, `weekly`, `every_*`) fetches over the top of it |
