...
def _fake_bin(tmp_path, skill_name="property-hunt", files=(), agent="property", src=None):
    ...
    fake_docker(tmp_path, home=tmp_path / "home" / f".hermes-{agent}", name=agent, running=False)
...
def test_add_skill_installs_into_the_agents_skills_directory(run, instance, tmp_path):
    ...
    r = run(
        "add-skill",
        "property",
        "plow-pbc/property-hunt",
        "--ref",
        "a" * 40,
        "--dest",
        "productivity/property-hunt",
...
def test_adding_the_same_skill_again_updates_the_pin_rather_than_duplicating(
...
def test_a_failed_manifest_publish_preserves_existing_pins(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        commands, "atomic_write", lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("full"))
    )
...
def test_a_branch_ref_is_refused(run, instance, tmp_path):
    run("register", "property", str(instance("property")))
    r = run(
        "add-skill", "property", "plow-pbc/property-hunt", "--ref", "main", env=_fake_bin(tmp_path)
    )
...
def test_a_fetched_script_is_executable(run, instance, tmp_path):
    ...
    script = (
        tmp_path / "home" / ".hermes-rowan" / "skills" / "plow-connectors" / "plow_connector.py"
    )
...
def test_a_subpath_install_takes_only_that_subtree(run, instance, tmp_path):
    ...
    r = run(
        ...
        "plow-connectors",
        "--src",
        "ref/hermes-skill/plow-connectors",
        env=_fake_bin(
            tmp_path,
            skill_name="plow-connectors",
            agent="rowan",
...
def test_a_traversing_destination_is_refused(run, instance, tmp_path):
    ...
    r = run(
        ...
        "--dest",
        "../../.ssh",
        env=_fake_bin(tmp_path, agent="rowan"),
    )
    assert r.returncode != 0
...
def test_restore_installs_every_fleet_skill(run, instance, tmp_path, dest, name):
...
def test_restore_skips_the_fleet_skill_when_the_instance_pins_it(run, instance, tmp_path):
    ...
    r = run(
        ...
        "property",
        env=_fake_bin(
            tmp_path, skill_name="google-workspace", files=(("INSTANCE.md", "instance copy"),)
        ),
    )
    assert r.returncode == 0, r.stderr
    assert "skipped" in r.stdout
    installed = (
        tmp_path / "home" / ".hermes-property" / "skills" / "productivity" / "google-workspace"
    )
...
def test_install_skill_skips_an_instance_pinned_dest_and_installs_the_rest(run, instance, tmp_path):
    """restore deliberately installs an instance's own skills.tsv copy last; a
    standalone fleet install over it would contradict the reviewed pin until
    the next restore silently flipped it back. With more than one fleet skill,
    the owned dest is skipped per skill while the others still install."""
    repo = instance("rowan")
    (repo / "skills.tsv").write_text(f"plow-pbc/x\t{'a' * 40}\tproductivity/google-workspace\t\n")
    run("register", "rowan", str(repo))
    run("restore", "rowan", env=_fake_bin(tmp_path, skill_name="google-workspace", agent="rowan"))
    r = run("install-skill", "rowan")
    assert r.returncode == 0, r.stderr
    assert "skipped" in r.stdout and "google-workspace" in r.stdout
    installed = (
        tmp_path / "home" / ".hermes-rowan" / "skills" / "growth" / "plow-invite" / "SKILL.md"
    )
    assert installed.exists()
