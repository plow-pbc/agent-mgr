...
def test_the_scaffolded_config_has_baseline_integrations_and_group_scope(
    run, tmp_path
):
    ...
    The model, and the compression fallback that must share its provider,
    are the image's: plow-init writes them on every boot from the seed, so a
    copy here could only ever be the one that drifted."""
    target = tmp_path / "acme-hermes-agent"
